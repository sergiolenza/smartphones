...
const renderProductsList = () =>
  render(
    <Router>
      <ProductsList onAddToCart={mockOnAddToCart} />
    </Router>
  );

describe('ProductsList view', () => {
  it('does render the productsList', async () => {
    mockedStatus.mockImplementation(() => 'success');
    mockedData.mockImplementation(() => mockProducts);
    renderProductsList();
    const productsList = await screen.findByRole('article', { name: 'products-list' });
    expect(productsList).toBeInTheDocument();
  });
  it('does render the LoadingPlaceholder when query status is loading', async () => {
    mockedStatus.mockImplementation(() => 'loading');
    mockedData.mockImplementation(() => mockProducts);
    renderProductsList();
    const loadingSvg = await screen.findByRole('img', { name: 'loading-placeholder' });
    expect(loadingSvg).toBeInTheDocument();
  });
  it('does render the ErrorPlaceholder when query status is error', async () => {
    mockedStatus.mockImplementation(() => 'error');
    renderProductsList();
    const errorPlaceholder = await screen.findByText('Oops!');
